// Code generated by qtc from "Edit.html". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

// <!-- Content managed by Project Forge, see [projectforge.md] for details. -->

//line views/vversion/Edit.html:2
package vversion

//line views/vversion/Edit.html:2
import (
	"github.com/kyleu/pftest/app"
	"github.com/kyleu/pftest/app/controller/cutil"
	"github.com/kyleu/pftest/app/util"
	"github.com/kyleu/pftest/app/version"
	"github.com/kyleu/pftest/views/components"
	"github.com/kyleu/pftest/views/layout"
)

//line views/vversion/Edit.html:11
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line views/vversion/Edit.html:11
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line views/vversion/Edit.html:11
type Edit struct {
	layout.Basic
	Model *version.Version
	IsNew bool
}

//line views/vversion/Edit.html:17
func (p *Edit) StreamBody(qw422016 *qt422016.Writer, as *app.State, ps *cutil.PageState) {
//line views/vversion/Edit.html:17
	qw422016.N().S(`
  <div class="card">
`)
//line views/vversion/Edit.html:19
	if p.IsNew {
//line views/vversion/Edit.html:19
		qw422016.N().S(`    <div class="right"><a href="/version/random"><button>Random</button></a></div>
    <h3>`)
//line views/vversion/Edit.html:21
		components.StreamSVGRefIcon(qw422016, `star`, ps)
//line views/vversion/Edit.html:21
		qw422016.N().S(` New Version</h3>
    <form action="/version/new" class="mt" method="post">
`)
//line views/vversion/Edit.html:23
	} else {
//line views/vversion/Edit.html:23
		qw422016.N().S(`    <div class="right"><a href="`)
//line views/vversion/Edit.html:24
		qw422016.E().S(p.Model.WebPath())
//line views/vversion/Edit.html:24
		qw422016.N().S(`/delete" onclick="return confirm('Are you sure you wish to delete version [`)
//line views/vversion/Edit.html:24
		qw422016.E().S(p.Model.String())
//line views/vversion/Edit.html:24
		qw422016.N().S(`]?')"><button>Delete</button></a></div>
    <h3>`)
//line views/vversion/Edit.html:25
		components.StreamSVGRefIcon(qw422016, `star`, ps)
//line views/vversion/Edit.html:25
		qw422016.N().S(` Edit Version [`)
//line views/vversion/Edit.html:25
		qw422016.E().S(p.Model.String())
//line views/vversion/Edit.html:25
		qw422016.N().S(`]</h3>
    <form action="" method="post">
`)
//line views/vversion/Edit.html:27
	}
//line views/vversion/Edit.html:27
	qw422016.N().S(`      <table class="mt expanded">
        <tbody>
          `)
//line views/vversion/Edit.html:30
	if p.IsNew {
//line views/vversion/Edit.html:30
		components.StreamTableInput(qw422016, "id", "", "ID", p.Model.ID, 5, "String text")
//line views/vversion/Edit.html:30
	}
//line views/vversion/Edit.html:30
	qw422016.N().S(`
          `)
//line views/vversion/Edit.html:31
	components.StreamTableInputNumber(qw422016, "revision", "", "Revision", p.Model.Revision, 5, "Integer")
//line views/vversion/Edit.html:31
	qw422016.N().S(`
          `)
//line views/vversion/Edit.html:32
	components.StreamTableInput(qw422016, "constcol", "", "Constcol", p.Model.Constcol, 5, "String text")
//line views/vversion/Edit.html:32
	qw422016.N().S(`
          `)
//line views/vversion/Edit.html:33
	components.StreamTableTextarea(qw422016, "varcol", "", "Varcol", 8, util.ToJSON(p.Model.Varcol), 5, "JSON object")
//line views/vversion/Edit.html:33
	qw422016.N().S(`
          <tr><td colspan="2"><button type="submit">Save Changes</button></td></tr>
        </tbody>
      </table>
    </form>
  </div>
`)
//line views/vversion/Edit.html:39
}

//line views/vversion/Edit.html:39
func (p *Edit) WriteBody(qq422016 qtio422016.Writer, as *app.State, ps *cutil.PageState) {
//line views/vversion/Edit.html:39
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/vversion/Edit.html:39
	p.StreamBody(qw422016, as, ps)
//line views/vversion/Edit.html:39
	qt422016.ReleaseWriter(qw422016)
//line views/vversion/Edit.html:39
}

//line views/vversion/Edit.html:39
func (p *Edit) Body(as *app.State, ps *cutil.PageState) string {
//line views/vversion/Edit.html:39
	qb422016 := qt422016.AcquireByteBuffer()
//line views/vversion/Edit.html:39
	p.WriteBody(qb422016, as, ps)
//line views/vversion/Edit.html:39
	qs422016 := string(qb422016.B)
//line views/vversion/Edit.html:39
	qt422016.ReleaseByteBuffer(qb422016)
//line views/vversion/Edit.html:39
	return qs422016
//line views/vversion/Edit.html:39
}
