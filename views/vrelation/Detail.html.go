// Code generated by qtc from "Detail.html". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line views/vrelation/Detail.html:1
package vrelation

//line views/vrelation/Detail.html:1
import (
	"github.com/kyleu/pftest/app"
	"github.com/kyleu/pftest/app/basic"
	"github.com/kyleu/pftest/app/controller/cutil"
	"github.com/kyleu/pftest/app/relation"
	"github.com/kyleu/pftest/views/components"
	"github.com/kyleu/pftest/views/components/view"
	"github.com/kyleu/pftest/views/layout"
)

//line views/vrelation/Detail.html:11
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line views/vrelation/Detail.html:11
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line views/vrelation/Detail.html:11
type Detail struct {
	layout.Basic
	Model          *relation.Relation
	BasicByBasicID *basic.Basic
	Paths          []string
}

//line views/vrelation/Detail.html:18
func (p *Detail) StreamBody(qw422016 *qt422016.Writer, as *app.State, ps *cutil.PageState) {
//line views/vrelation/Detail.html:18
	qw422016.N().S(`
  <div class="card">
    <div class="right">
      <a href="#modal-relation"><button type="button" title="JSON">`)
//line views/vrelation/Detail.html:21
	components.StreamSVGButton(qw422016, "code", ps)
//line views/vrelation/Detail.html:21
	qw422016.N().S(`</button></a>
      <a href="`)
//line views/vrelation/Detail.html:22
	qw422016.E().S(p.Model.WebPath(p.Paths...))
//line views/vrelation/Detail.html:22
	qw422016.N().S(`/edit" title="Edit"><button>`)
//line views/vrelation/Detail.html:22
	components.StreamSVGButton(qw422016, "edit", ps)
//line views/vrelation/Detail.html:22
	qw422016.N().S(`</button></a>
    </div>
    <h3>`)
//line views/vrelation/Detail.html:24
	components.StreamSVGIcon(qw422016, `star`, ps)
//line views/vrelation/Detail.html:24
	qw422016.N().S(` `)
//line views/vrelation/Detail.html:24
	qw422016.E().S(p.Model.TitleString())
//line views/vrelation/Detail.html:24
	qw422016.N().S(`</h3>
    <div><a href="`)
//line views/vrelation/Detail.html:25
	qw422016.E().S(relation.Route(p.Paths...))
//line views/vrelation/Detail.html:25
	qw422016.N().S(`"><em>Relation</em></a></div>
    `)
//line views/vrelation/Detail.html:26
	StreamDetailTable(qw422016, p, ps)
//line views/vrelation/Detail.html:26
	qw422016.N().S(`
  </div>
`)
//line views/vrelation/Detail.html:29
	qw422016.N().S(`  `)
//line views/vrelation/Detail.html:30
	components.StreamJSONModal(qw422016, "relation", "Relation JSON", p.Model, 1)
//line views/vrelation/Detail.html:30
	qw422016.N().S(`
`)
//line views/vrelation/Detail.html:31
}

//line views/vrelation/Detail.html:31
func (p *Detail) WriteBody(qq422016 qtio422016.Writer, as *app.State, ps *cutil.PageState) {
//line views/vrelation/Detail.html:31
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/vrelation/Detail.html:31
	p.StreamBody(qw422016, as, ps)
//line views/vrelation/Detail.html:31
	qt422016.ReleaseWriter(qw422016)
//line views/vrelation/Detail.html:31
}

//line views/vrelation/Detail.html:31
func (p *Detail) Body(as *app.State, ps *cutil.PageState) string {
//line views/vrelation/Detail.html:31
	qb422016 := qt422016.AcquireByteBuffer()
//line views/vrelation/Detail.html:31
	p.WriteBody(qb422016, as, ps)
//line views/vrelation/Detail.html:31
	qs422016 := string(qb422016.B)
//line views/vrelation/Detail.html:31
	qt422016.ReleaseByteBuffer(qb422016)
//line views/vrelation/Detail.html:31
	return qs422016
//line views/vrelation/Detail.html:31
}

//line views/vrelation/Detail.html:33
func StreamDetailTable(qw422016 *qt422016.Writer, p *Detail, ps *cutil.PageState) {
//line views/vrelation/Detail.html:33
	qw422016.N().S(`
  <div class="mt overflow full-width">
    <table>
      <tbody>
        <tr>
          <th class="shrink" title="UUID in format (00000000-0000-0000-0000-000000000000)">ID</th>
          <td>`)
//line views/vrelation/Detail.html:39
	view.StreamUUID(qw422016, &p.Model.ID)
//line views/vrelation/Detail.html:39
	qw422016.N().S(`</td>
        </tr>
        <tr>
          <th class="shrink" title="UUID in format (00000000-0000-0000-0000-000000000000)">Basic ID</th>
          <td class="nowrap">
            `)
//line views/vrelation/Detail.html:44
	if x := p.BasicByBasicID; x != nil {
//line views/vrelation/Detail.html:44
		qw422016.N().S(`
            `)
//line views/vrelation/Detail.html:45
		qw422016.E().S(x.TitleString())
//line views/vrelation/Detail.html:45
		qw422016.N().S(` <a title="Basic" href="`)
//line views/vrelation/Detail.html:45
		qw422016.E().S(x.WebPath(p.Paths...))
//line views/vrelation/Detail.html:45
		qw422016.N().S(`">`)
//line views/vrelation/Detail.html:45
		components.StreamSVGLink(qw422016, `star`, ps)
//line views/vrelation/Detail.html:45
		qw422016.N().S(`</a>
            `)
//line views/vrelation/Detail.html:46
	} else {
//line views/vrelation/Detail.html:46
		qw422016.N().S(`
            `)
//line views/vrelation/Detail.html:47
		view.StreamUUID(qw422016, &p.Model.BasicID)
//line views/vrelation/Detail.html:47
		qw422016.N().S(`
            `)
//line views/vrelation/Detail.html:48
	}
//line views/vrelation/Detail.html:48
	qw422016.N().S(`
          </td>
        </tr>
        <tr>
          <th class="shrink" title="String text">Name</th>
          <td><strong>`)
//line views/vrelation/Detail.html:53
	view.StreamString(qw422016, p.Model.Name)
//line views/vrelation/Detail.html:53
	qw422016.N().S(`</strong></td>
        </tr>
        <tr>
          <th class="shrink" title="Date and time, in almost any format">Created</th>
          <td>`)
//line views/vrelation/Detail.html:57
	view.StreamTimestamp(qw422016, &p.Model.Created)
//line views/vrelation/Detail.html:57
	qw422016.N().S(`</td>
        </tr>
      </tbody>
    </table>
  </div>
`)
//line views/vrelation/Detail.html:62
}

//line views/vrelation/Detail.html:62
func WriteDetailTable(qq422016 qtio422016.Writer, p *Detail, ps *cutil.PageState) {
//line views/vrelation/Detail.html:62
	qw422016 := qt422016.AcquireWriter(qq422016)
//line views/vrelation/Detail.html:62
	StreamDetailTable(qw422016, p, ps)
//line views/vrelation/Detail.html:62
	qt422016.ReleaseWriter(qw422016)
//line views/vrelation/Detail.html:62
}

//line views/vrelation/Detail.html:62
func DetailTable(p *Detail, ps *cutil.PageState) string {
//line views/vrelation/Detail.html:62
	qb422016 := qt422016.AcquireByteBuffer()
//line views/vrelation/Detail.html:62
	WriteDetailTable(qb422016, p, ps)
//line views/vrelation/Detail.html:62
	qs422016 := string(qb422016.B)
//line views/vrelation/Detail.html:62
	qt422016.ReleaseByteBuffer(qb422016)
//line views/vrelation/Detail.html:62
	return qs422016
//line views/vrelation/Detail.html:62
}
