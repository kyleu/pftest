...
func (s *Service) GetMultiple(ctx context.Context, tx *sqlx.Tx, logger util.Logger, ids ...uuid.UUID) (Seeds, error) {
	if len(ids) == 0 {
		return Seeds{}, nil
	}
...
func (s *Service) ListSQL(ctx context.Context, tx *sqlx.Tx, sql string, logger util.Logger) (Seeds, error) {
	ret := dtos{}
