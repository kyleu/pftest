...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Path) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Path, _ int) {
		model.Created = util.TimeCurrent()
	})
	q := database.SQLInsert(tableQuoted, columnsQuoted, len(models), s.db.Type)
	vals := lo.FlatMap(models, func(arg *Path, _ int) []any {
		return arg.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, vals...)
}

func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Path) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			...
			logger.Infof("creating paths [%d-%d]", idx*chunkSize, count)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Path, logger util.Logger) error {
	curr, err := s.Get(ctx, tx, model.ID, logger)
	if err != nil {
		return errors.Wrapf(err, "can't get original path [%s]", model.String())
	}
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Path) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Path, _ int) {
	...
	data := lo.FlatMap(models, func(model *Path, _ int) []any {
		return model.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, data...)
}

func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Path) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
			...
			logger.Infof("saving paths [%d-%d]", idx*chunkSize, count)
		}
