...
func (s *Service) ListWhere(ctx context.Context, tx *sqlx.Tx, where string, params *filter.Params, logger util.Logger, values ...any) (Paths, error) {
	params = filters(params)
...
func (s *Service) Search(ctx context.Context, query string, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (Paths, error) {
	...
	if strings.Contains(query, ":") {
		wc, vals, err = database.QueryFieldDescs(PathFieldDescs, query, 0)
	} else {
		wc = searchClause
