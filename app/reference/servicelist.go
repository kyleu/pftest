...
func (s *Service) ListWhere(ctx context.Context, tx *sqlx.Tx, where string, params *filter.Params, logger util.Logger, values ...any) (References, error) {
	params = filters(params)
...
func (s *Service) Search(ctx context.Context, query string, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (References, error) {
	...
	if strings.Contains(query, ":") {
		wc, vals, err = database.QueryFieldDescs(ReferenceFieldDescs, query, 0)
	} else {
		wc = searchClause
...
func (s *Service) SearchEntries(ctx context.Context, query string, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (result.Results, error) {
	...
	return lo.Map(ret, func(m *Reference, _ int) *result.Result {
		return result.NewResult("Reference", m.String(), m.WebPath(), m.TitleString(), "star", m, m, query)
	}), nil
}
