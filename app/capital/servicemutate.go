...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*Capital) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			logger.Infof("saving capitals [%d-%d]", idx*chunkSize, ((idx+1)*chunkSize)-1)
		}
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, logger util.Logger, models ...*Capital) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
