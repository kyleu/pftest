...
func (s *Service) List(ctx context.Context, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (Basics, error) {
...
func (s *Service) ListSQL(ctx context.Context, tx *sqlx.Tx, sql string, logger util.Logger, values ...any) (Basics, error) {
	ret := rows{}
...
func (s *Service) ListWhere(ctx context.Context, tx *sqlx.Tx, where string, params *filter.Params, logger util.Logger, values ...any) (Basics, error) {
	params = filters(params)
...
func (s *Service) Search(ctx context.Context, query string, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (Basics, error) {
	params = filters(params)
	var wc string
	var vals []any
	var err error
	if strings.Contains(query, ":") {
		wc, vals, err = database.QueryFieldDescs(BasicFieldDescs, query, 0)
	} else {
		wc = searchClause
...
func (s *Service) SearchEntries(ctx context.Context, query string, tx *sqlx.Tx, params *filter.Params, logger util.Logger) (result.Results, error) {
	...
	return lo.Map(ret, func(m *Basic, _ int) *result.Result {
		return result.NewResult("Basic", m.String(), m.WebPath(), m.TitleString(), "star", m, m, query)
	}), nil
}
