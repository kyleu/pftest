...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Softdel) error {
	if len(models) == 0 {
		return nil
	}
...
func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Softdel) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			...
			logger.Infof("creating softdels [%d-%d]", idx*chunkSize, count)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Softdel, logger util.Logger) error {
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Softdel) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Softdel, _ int) {
...
func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Softdel) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
			...
			logger.Infof("saving softdels [%d-%d]", idx*chunkSize, count)
		}
