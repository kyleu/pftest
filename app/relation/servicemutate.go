...
func (s *Service) Create(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Relation) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Relation, _ int) {
		model.Created = util.TimeCurrent()
	})
	q := database.SQLInsert(tableQuoted, columnsQuoted, len(models), s.db.Type)
	vals := lo.FlatMap(models, func(arg *Relation, _ int) []any {
		return arg.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, vals...)
}

func (s *Service) CreateChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Relation) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			...
			logger.Infof("creating relations [%d-%d]", idx*chunkSize, count)
		}
...
func (s *Service) Update(ctx context.Context, tx *sqlx.Tx, model *Relation, logger util.Logger) error {
...
func (s *Service) Save(ctx context.Context, tx *sqlx.Tx, logger util.Logger, models ...*Relation) error {
	if len(models) == 0 {
		return nil
	}
	lo.ForEach(models, func(model *Relation, _ int) {
	...
	data := lo.FlatMap(models, func(model *Relation, _ int) []any {
		return model.ToData()
	})
	return s.db.Insert(ctx, q, tx, logger, data...)
}

func (s *Service) SaveChunked(ctx context.Context, tx *sqlx.Tx, chunkSize int, progress *util.Progress, logger util.Logger, models ...*Relation) error {
	for idx, chunk := range lo.Chunk(models, chunkSize) {
		if logger != nil {
			count := ((idx + 1) * chunkSize) - 1
			if len(models) < count {
			...
			logger.Infof("saving relations [%d-%d]", idx*chunkSize, count)
		}
