...
func (s *Service) ListSQL(ctx context.Context, tx *sqlx.Tx, sql string, logger util.Logger, values ...any) (Versions, error) {
	ret := rows{}
...
func (s *Service) Random(ctx context.Context, tx *sqlx.Tx, logger util.Logger) (*Version, error) {
	ret := &row{}
	q := database.SQLSelect(columnsString, tableQuoted, "", "random()", 1, 0, s.db.Placeholder())
	err := s.db.Get(ctx, ret, q, tx, logger)
	if err != nil {
		return nil, errors.Wrap(err, "unable to get random versions")
	}
	return ret.ToVersion(), nil
}
