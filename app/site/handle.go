...
func Handle(path []string, as *app.State, ps *cutil.PageState) (string, layout.Page, []string, error) {
	...
	case util.AppKey:
		msg := "\n  " +
			...
			"<meta name=\"go-source\" content=\"github.com/kyleu/pftest %s %s/tree/main{/dir} %s/blob/main{/dir}/{file}#L{line}\">"
	...
	case keyDownload:
		...
		data := util.ValueMap{"base": "https://github.com/kyleu/pftest/releases/download/v" + as.BuildInfo.Version, "links": dls}
